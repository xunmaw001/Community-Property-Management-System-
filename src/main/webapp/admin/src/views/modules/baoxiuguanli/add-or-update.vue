            ...
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.guzhangmiaoshu" label="故障描述" prop="guzhangmiaoshu">
                    <span v-html="ruleForm.guzhangmiaoshu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.guzhangmiaoshu = this.ruleForm.guzhangmiaoshu.replace(reg,'../../../ssm7vwc6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `baoxiuguanli/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.baoxiuguanliCrossAddOrUpdateFlag = false;
                  this.parent.search();
