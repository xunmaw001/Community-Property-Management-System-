<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="投诉人员" prop="tousurenyuan">
          <el-select v-model="ruleForm.tousurenyuan" placeholder="请选择投诉人员">
            <el-option
                v-for="(item,index) in tousurenyuanOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="投诉人员" prop="tousurenyuan">
	      <el-input v-model="ruleForm.tousurenyuan"
                placeholder="投诉人员" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shouji = true;
          ...
          if(o=='tousurenyuan'){
            this.ruleForm.tousurenyuan = obj[o];
	    this.ro.tousurenyuan = true;
            continue;
          }
          if(o=='tousuneirong'){
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.tousurenyuanOptions = "保安,维修员".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `tousuguanli/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.tousuneirong = this.ruleForm.tousuneirong.replace(reg,'../../../ssm7vwc6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tousuguanli/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tousuguanliCrossAddOrUpdateFlag = false;
                  this.parent.search();
